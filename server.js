...
app.get('/',async (request, response)=>{
    //.find = finding all of these objects from the database and turning them into an array
        const weeklyToDoItem = await db.collection('weeklyToDoItem').find().toArray()
        const weeklyItemsLeft = await db.collection('weeklyToDoItem').countDocuments({completed: false})
    /*Pass the array of objects into our EJS template which spits out HTML that the server responds with that 
    HTML and that is how we can see the objects on the client side. */
        response.render('index.ejs', { weeklyItems: weeklyToDoItem, weeklyLeft: weeklyItemsLeft })
        // db.collection('todos').find().toArray()
        // .then(data => {
        //     db.collection('todos').countDocuments({completed: false})
        //     .then(itemsLeft => {
...
//Weekly todo
app.post('/addWeeklyTodo', (request, response) => {
    //Goes to database and adds a new document to the todo collection. 
    db.collection('weeklyToDoItem').insertOne({thing: request.body.weeklyToDoItem, completed: false}) //the request body comes from the form. The "thing" will be the new list item in the form of a document in our collection.
    .then(result => {
        console.log('Weekly Todo Added') // we can see in the console if adding the item from the form was successful.
        response.redirect('/') //respond with a refresh to trigger a new get which goes to our collection and shows the new document.
    })
    .catch(error => console.error(error))
})
    ...
    app.put('/markComplete', (request, response) => {
        //search the database for a document that matches the item in the body. 
        db.collection('weeklyToDoItem').updateOne({thing: request.body.weeklyToDoItemFromJS},{ //the request body comes from the form. 
            $set: {
                completed: true
              }
        },{
    ...
    //seach are database or collection for a matching document from the request body
    app.put('/markUnComplete', (request, response) => {
        db.collection('weeklyToDoItem').updateOne({thing: request.body.weeklyToDoItemFromJS},{ 
            $set: {
                completed: false
              }
        },{
    ...
    //DE-LE-TE part of CRUD
    //Smurf from our client side JavaScript Main.js listening here
    app.delete('/deleteWeeklyItem', (request, response) => {
        db.collection('weeklyToDoItem').deleteOne({thing: request.body.weeklyToDoItemFromJS})
        .then(result => {
            console.log('Todo Deleted') //tells us in the console log that the object was deleted. 
            response.json('Todo Deleted')
        })
